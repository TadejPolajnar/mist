...
**编译器强制执行的规则：**只能通过 `x.value...` 路径变更。在非箭头回调（`success(res) { ... }`）中写状态是 M1030 错误——它们会重绑 `this`；请用箭头函数。handler 可以是 `function` 声明，也可以是顶层 `const name = (...) => {...}` 箭头——两者都会成为页面方法。写入立即生效：写完后立刻读 `x.value` 能看到新值（`setData` 仍按 handler 批量合并一次）。使用响应式名称而不带 `.value` 是 M1007 错误。取别名到局部变量再通过它写入（`const t = todos.value[0]; t.done = true`）——或通过 `for...of` 变量、`forEach` 回调参数变更——是 M1001 错误，附带修复提示。除 `push`/索引赋值以外的数组方法是 M1004 错误。缺少 `key=` 的 `.map()` 产生警告（M1008）：它会回退为整数组写入。
...
**绑定**——`{expr}` → `{{expr}}`，并剥离 `.value`。无法解析为表达式的花括号文本（`{ return v }`、代码片段、WXS 体）会保留为字面文本，而不会变成数据绑定。成员访问、算术、比较、三元、`&&`/`||`、字符串拼接都在 WXML 中内联执行。**WXML 无法求值的内容会自动提升**——函数调用（包括 `Math.*` 和带非响应式参数的调用）、模板字符串和可选链编译为生成的派生值，每批次重算；循环内的 `{fmtDate(t.ts)}` 成为每个条目上的计算字段（`t._c0`），并保留键控 diff。限制（M1009 错误，绝不静默）：*嵌套*循环内的调用，以及引用循环条目的模板字符串/可选链——请把它们预先计算为计算字段或派生值。
...
在 `app.mist` 声明一次即可让所有页面和组件（包括被内联的组件）继承
这个下限；单元自己声明的 `minLibVersion` 优先。设置后，使用文档最低版本更高的特性——`refresher-enabled` 需要
2.10.1、`value:bind` 需要 2.9.3——会带精确版本号警告。不设置则不做版本
检查。仅编译期存在，永远不会进入 `.json`；请与微信管理后台设置的最低
基础库保持一致。
...
- **`<style global>`** 是反方向的选择：样式块被提升进 `app.wxss` 而不是
  ...
  `styleIsolation` 同时改为 `'apply-shared'`——否则微信的样式隔离会挡住
  组件自己那些已提升到 app 层的规则。`config` 中显式声明的
  `styleIsolation` 仍然优先，但 `'isolated'` 与 `global` 同用是编译
  错误：隔离会把已提升的规则挡在组件之外，让它们死在 `app.wxss` 里。
  适用场景：内联组件刻意为父级/插槽内容提供样式，或组件主动接入页面级
  CSS。同一个块上同时写 `scoped` 和 `global` 是错误，`<style>` 上的
  未知属性（包括拼写错误）也是错误；`app.mist` 会拒绝该属性（它的样式
  本来就是全局的）。单文件构建没有 `app.wxss`——此时样式块保留在单元
  自己的产物中，隔离默认值也不改变。
