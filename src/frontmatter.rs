...
pub fn emit_js_route(
    ...
    if is_page {
        ...
        let route_guard = route_seed.map(|(param, _)| {
            format!(
                "    if (__q.{param} === undefined) {{\n      console.error(\"[mist] missing route param '{param}' — navigate to this page with {{ {param}: ... }}\");\n      wx.navigateBack({{ delta: 1 }});\n      return;\n    }}",
                param = param
            )
        });
        let route_set = route_seed.map(|(param, bound)| {
            if bound {
                format!("this.__set('{param}', __q.{param});", param = param)
            } else {
                format!(";(this._{param} = __q.{param}, rt.touch(this, '{param}'));", param = param)
            }
        });
        ...
        for l in &analysis.lifecycles {
            ...
            if l.hook == "onLoad" {
                has_on_load = true;
                if let (Some(guard), Some(seed)) = (&route_guard, &route_set) {
                    let call = if l.is_async {
                        format!("await (async {} => {})(__q);", l.params, l.body)
                    } else {
                        format!(";({} => {})(__q);", l.params, l.body)
                    };
                    out.push_str(&format!(
                        "  {}onLoad(__q) {{\n{}\n    {}rt.init(this);\n    {}\n    {}\n  }},\n",
                        prefix, guard, bind_stmt, seed, call
                    ));
                    continue;
                }
        ...
        if !has_on_load {
            match (&route_guard, &route_set) {
                (Some(guard), Some(seed)) => out.push_str(&format!(
                    "  onLoad(__q) {{\n{}\n    {}rt.init(this);\n    {}\n  }},\n",
                    guard, bind_stmt, seed
                )),
