...
| `runtime/mist-rt.js` | ~380 | `set/touch/flush` (microtask-batched setData; `touch(page, name?)` = derive-only flush for unbound state), `derive(page, out, name, key, compute, deps)` (keyed **field-level** diff vs `__prev` snapshots; **per-derived dirty bits** — `deps` from `frontmatter::derived_deps` (resolves through pure methods and store accessors), clean deriveds skipped, changed deriveds re-dirty for chains; `deps` omitted/null ⇒ always recompute; **transactional flush** — mirror/`__prev` roll back if `setData` throws, next flush recomputes all; **budget chunking** — multi-key batches over ~900KB split into sequential in-order `setData` calls via `send` (`setDataBudget(bytes)` overrides); a batch containing any single over-budget entry is sent whole for clean whole-batch rejection; sizing is UTF-8-byte-accurate (`utf8Len`) so chunks can't be size-rejected mid-sequence — only non-size WeChat errors can partially apply a chunked flush, recovered via rollback + resync + next-flush recompute), `applyPath`, `store/bindStores/unbindStores` (cross-page path-precise notifications), `observePerf/perfEntries` (launch metrics, wired into generated `app.js`). |
...
diagnostics, and serves completions (frontmatter symbols in `{…}` bindings, plus template completions from `tag_meta` — a textual `template_context` scanner classifies the cursor as tag-name vs attribute position and serves native tags/aliases/imported components, per-tag attributes + `onXxx` events, and imported components' props read from their `.mist` files), hover, go-to-definition (store symbols
...
| `cli.rs` | --version/--help, unknown-command suggestion, init scaffold compiles, mist-routes.d.ts emission, `mistc test` run/filter/timeout/watch-rerun |
